<?php
	// Connect to server and select database.
	$tbl_name="topic"; // Table name
?>
<!DOCTYPE HTML>
<html>
	<head>
		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1.0">
		<title>Bookie Wookie - Home</title>
		{{ HTML::style('css/base.css') }}
		{{ HTML::style('css/base-responsive.css') }}
	</head>

	<body>
		<div class="navbar navbar-fixed-top">
			<div class="navbar-inner">
                <div class="container">
                    <a class="btn btn-navbar" data-toggle="collapse" data-target=".nav-collapse">
                        Menu<!-- <span class="icon-th-list"> -->
                    </a>
                    <a href="{{ URL::to('/') }}" class="brand">Bookie Wookie</a>

                    <?php if(Auth::check()) { ?>
                        <div class="btn-group pull-right">
                            <button class="btn">Welcome {{ $user->firstname }}</button>
                            <button class="btn dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></button>
                            <ul class="dropdown-menu">
                                {{ HTML::linkRoute('sessions.logout', 'Logout') }}
                                <li><a href="{{ URL::to('logout') }}">Logout</a></li>
                            </ul>
                        </div>
                    <?php } ?>

                    <div class="nav-collapse collapse">

                        <ul class="nav pull-right">
                            <li class="active"><a href="index.php">Home</a></li>
                            <li><a href="bookexchange.blade.php">Book Exchange</a></li>
                            <? if (Auth::check()) {
                                echo HTML::linkRoute('pages.members', 'Members');
                            } ?>
                            <li>{{ HTML::linkRoute('pages.about', 'Logout') }}</li>
                            <li><a href="{{ URL::to('pages.about') }}">About Us</a></li>

                        </ul>
                    </div>
                </div>
			</div>
		</div>

		<div class="hero-unit">
			<h2> Welcome to Bookie Wookie </h2>
			<p>The one stop shop for all the books you need. </p>
		</div>

		<div class="container">
			<div class="row-fluid">
				<div class="span8 well" id="index">
					<p>Bookie Wookie is an online book exchange platform designed to help students enrolled in the CST program @ BCIT save money by allowing them to buy, sell or trade used text books. Anyone can join and members can connect directly with buyers or sellers through online classified ads.Help us spread the word by telling your class mates about our site. To get things started, <? if (isLoggedIn()){ echo ''; } else { echo ' <a href="#register" role="button" data-toggle="modal">join</a> our site, create a profile, '; } ?><a href="#postAd" role="button" data-toggle="modal">post</a> an add and wait to be contacted!</p>

					<div class="row-fluid">
						<?php if(!Auth::check()) {?>
						    <div class="span4 well">
                                <h4>Become a member / Login</h4>
                                <p>Join us to get started, create a <a href="#register" role="button" data-toggle="modal">profile</a> or <a  href="#login" role="button" data-toggle="modal">login</a> if you are already an existing member.<p>
                                <a href="#register" class="btn btn-primary" role="button" data-toggle="modal">Join</a>
                                <a href="#login" class="btn btn-primary" role="button" data-toggle="modal">Login</a>
						    </div>
                        <?php } ?>

						<div class="span4 well">
						<h4>Search Books</h4>
							<p>Check out the ads in our {{ HTML::linkRoute('pages.bookexchange', 'Book Exchange') }} section and search for the book your are looking for!<p>
							{{ HTML::linkRoute('pages.bookexchange', 'Browse', null, array('class' => 'btn btn-primary')) }}
						</div>
						<div class="span4 well">
						    <h4>FAQ</h4>
							<p>Check out the {{ HTML::linkRoute('pages.about', 'Frequently Asked Questions') }} section to get a better understanding of how to use this site.<p>
							{{ HTML::linkRoute('pages.faq', 'FAQ', null, 'btn btn-primary') }}
						</div>
					</div>

				</div>
				<div class="span4">
					<ul class="nav nav-list well">
						<li class="nav-header">More Info</li>
						<li>{{ HTML::linkRoute('pages.resources', 'Resources') }}</li>
						<li>{{ HTML::linkRoute('pages.faq', 'FAQ') }}</li>
						<li>{{ HTML::linkRoute('pages.privacy', 'Privacy Policy') }}</li>
						<li>{{ HTML::linkRoute('pages.contact', 'Contact Us') }}</li>
						<li class="divider"></li>
						<li>{{ HTML::linkRoute('pages.termbook', 'Books by term') }}</li>
					</ul>
				</div>
			</div>
			<hr>
			<footer style="text-align:center">
			<p>&copy; {{ HTML::linkRoute('pages.index', 'BookieWookie.com') }} 2013 || Website Designed by CST Students : {{ HTML::linkRoute('pages.about', 'Jas, Shah, Daniel and Randeep') }}</p>
			</footer>
		</div>

		<!-- modals -->

		<div id="contactUs" class="modal hide fade" aria-labelledby="modalLabel" aria-hidden="true">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i></button>
					<h3 id="modalLabel">Contact Us</h3>
			</div>

			<div class="modal-body">
					<div class="controls controls-row">
						<form>
    						<label>Name</label>
    						<input type="text" placeholder="Name">
    						<label>E-mail</label>
    						<input type="email" placeholder="e-mail">
    						<label>Request/Inquiry</label>
    						<textarea rows="3"></textarea>
    						<span class="help-block">Thank you, we will get back to you as soon as possible.</span>
    						<button type="submit" class="btn btn-primary">Submit</button>
    					</form>
					</div>
			</div>
			<div class="modal-footer">
				<button class="btn btn-danger" data-dismiss="modal" aria-hidden="true">cancel</button>
				<button class="btn btn-success" >Send</button>
			</div>

		</div>
		<div id="login" class="modal hide fade" aria-labelledby="modalLabel" aria-hidden="true">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i></button>
				<h3 id="modalLabel">Login</h3>
			</div>

            {{ Form::open(['route' => 'sessions.store']) }}
                <div class="modal-body">
                    {{ Form::text('username', null, array('placeholder' => 'Username', 'id' => 'txtUSRName', 'class' => 'input-block-level')) }} <!-- Add onBlur jquery methods in js file not here -->
                    {{ Form::password('password', array('placeholder' => 'Password', 'id' => 'txtPSWRD', 'class' => 'input-block-level')) }} <!-- onblur="testPSWRD('txtPSWRD', 'loginPass') -->
                    {{ Form::checkbox('remember') }}
                </div>

                <div class="modal-footer">
                    {{ Form::submit('Login', array('class' => 'btn btn-success')) }}
                    <button class="btn btn-danger" data-dismiss="modal" aria-hidden="true">cancel</button>
                </div>
			{{ Form::close() }}

		</div><!-- Modal form end-->

		<div id="register" class="modal hide fade" aria-labelledby="modalLabel" aria-hidden="true">

			{{ Form::open(['route' => 'users.store']) }}
                <div class="modal-header">
                    <button class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i></button>
                    <h3 id="modalLabel">Register</h3>
                </div>

                <div class="modal-body">
                    <div class="controls controls-row">
                        {{ Form::text('fname', null, array('placeholder' => 'First Name', 'id' => 'txtFirstName', 'class' => 'span2')) }} <!-- onblur="testName('txtFirstName')"> -->
                        {{ Form::text('lname', null, array('placeholder' => 'Last Name', 'id' => 'txtLastName', 'class' => 'span2')) }}<!-- onblur="testName('txtLastName')"> -->
                    </div>

                    <div class="controls control-group">

                        <label id="registerUsername"><? echo $loginerror; ?></label>
                        {{ Form::text('username', null, array('placeholder' => 'Username', 'id' => 'txtUsername', 'class' => 'span5')) }}<!-- onblur="testLoginUserName('txtUsername','registerUsername')"> -->
                        <label id="registerPassword"></label>
                        {{ Form::password('password', null, array('placeholder' => 'Password', 'id' => 'txtPassword', 'class' => 'span5')) }}<!-- onblur="testLoginPassword('txtPassword','registerPassword')"> -->
                        <label id="registerConfirmPass"></label>
                        {{ Form::password('confirm-password', null, array('placeholder' => 'Confirm Password', 'id' => 'txtConfirmPassword', 'class' => 'span5')) }}<!-- onblur="confirmPass('txtConfirmPassword','txtPassword','registerConfirmPass')"> -->
                        <label id="registerEmail"></label>
                        {{ Form::email('email', null, array('placeholder' => 'Email Address', 'id' => 'txtEmail', 'class' => 'span5')) }}<!--  onblur="testEmail('txtEmail','registerEmail')"> -->
                    </div>
                </div>

                <div class="modal-footer">
                    <button class="btn btn-danger" data-dismiss="modal" aria-hidden="true">cancel</button>
                    {{ Form::submit('Register', array('class' => 'btn btn-success')) }}
                </div>
			{{ Form::close() }}

		</div><!-- end modal register -->

		<div id="postAd" class="modal hide fade" aria-labelledby="modalLabel" aria-hidden="true">
			<div class="modal-header">
				<button class="close" data-dismiss="modal" aria-hidden="true"><i class="icon-remove"></i></button>
				<h3 id="modalLabel">Post an ad</h3>
			</div>

			<div class="modal-body">
                <div class="controls controls-row">
                    {{ Form::open(['route' => 'postings.store', 'class' => 'form-horizontal']) }}
                        <label id="bookLabel"></label>
                        {{ Form::text('book', null, array('id' => 'txtBook', 'placeholder' => 'Name of Book', 'class' => 'span5')) }} <!-- onblur="testName('txtBook','bookLabel')" -->
                        <br><br>
                        <label id="conditionLabel"></label>
                        <!-- onblur="checkPosition('selCondition','conditionLabel')"> -->
                        {{ Form::select('selCondition', [
                            'Poor' => 'Poor',
                            'Good' => 'Good',
                            'Excellent' => 'Excellent'
                        ]), 'Good' }} <!-- array('class' => 'span5', 'id' => 'selCondition') -->
                        <br><br>
                        <label id="priceLabel"></label>
                        <!-- onblur="checkPosition('selPrice','priceLabel')"> -->
                        {{ Form::select('selPrice', [
                            'free'      => 'Free',
                            'ten'       => '$10',
                            'twenty'    => '$20',
                            'thirty'    => '$30',
                            'forty'     => '$40',
                            'fifty'     => '$50',
                            'sixty'     => '$60',
                            'seventy'   => '$70',
                            'eighty'    => '$80',
                            'ninety'    => '$90',
                            'hundred'   => '$100+'
                        ]), 'twenty' }} <!-- array('id' => 'selPrice')  -->
                        <br><br>
                    {{ Form::close() }}
                </div>
            </div>
			<div class="modal-footer">
				<button class="btn btn-danger" data-dismiss="modal" aria-hidden="true">cancel</button>
				<button class="btn btn-success" >post</button>
			</div>

		</div> <!--end modal post ad -->

		{{ HTML::script('http://code.jquery.com/jquery-1.9.0.min.js') }}
		{{ HTML::script('js/scripts') }}

		<script type="text/javascript">

			function $(id){
				var element = document.getElementById(id);
				if( element == null )
					alert( 'Programmer error: ' + id + ' does not exist.' );
				return element;
			}

			function testUSRName(name, element) {
				var username = $(name).value;
				var usernamelength = $(name).value.length;
				var label = $(element);
  				if (usernamelength <= 0 ) {
   			 		label.innerHTML = "Please enter your username to login";
   			 		label.style.color = "red";
 			 	}
				else {
					label.innerHTML = "";

				}

			}

			function testPSWRD(pass , element) {
				var password = $(pass).value.length;
				var label = $(element);
  				if ( password <= 0 ) {
   			 		label.innerHTML="Please enter your password to login";
   			 		label.style.color="red";
 			 	}
				else {
					label.innerHTML = "";

				}
			}

			function testName(name) {
  				if ($(name).value == '' ) {
   			 		$(name).placeholder="Incomplete";
   			 		$(name).style.color="red";
 			 	}else{
 			 		$(name).style.color="#555555"
 			 	}
			}

			function testLoginUserName(name, element) {
				var username = $(name).value;
				var usernamelength = $(name).value.length;
				var label = $(element);
  				if (usernamelength < 5 ) {
   			 		label.innerHTML = "Invalid username, must be 5 characters";
   			 		label.style.color = "red";
 			 	}
 			 	else{
 			 		label.innerHTML = "Valid username";
 			 		label.style.color ="green";
 			 	}
			}

			function testLoginPassword(pass , element) {
				var password = $(pass).value.length;
				var label = $(element);
  				if (password < 8 ) {
   			 		label.innerHTML="Please enter a password of at least 8 characters";
   			 		label.style.color="red";
 			 	}else {
 			 		label.innerHTML="Valid password";
 			 		label.style.color="green";
 			 	}
			}

			function confirmPass(pass_0, pass_1, element){
				 var first = $(pass_0).value;
				 var second = $(pass_1).value;
				 var label = $(element);

				if ( second.length>8 && second == first ){
					label.innerHTML="Passwords matched";
					label.style.color="green";
					}

				else if (second.length>8 && second != first) {
					label.innerHTML="Passwords did not match";
					label.style.color="red";
					}
				else  {
					label.innerHTML="Enter a Password first which is at least 8 characters long";
					label.style.color="red";
					}



			}

			function checkEmail(email) {
				var pattern = /[a-z | A-Z | 0-9]+@{1}[a-z | A-Z | 0-9]+\.{1}(com|ca|org)$/i;
				return pattern.test($(email).value);
			}

			function testEmail(email, element) {
				var label = $(element);
				if ( !checkEmail(email) ) {
					label.innerHTML="Please enter a valid e-mail";
					label.style.color="red";
				} else {
					label.innerHTML="Valid e-mail";
					label.style.color="green";
					return ('true');
				}
			}



		</script>

	</body>
</html>
